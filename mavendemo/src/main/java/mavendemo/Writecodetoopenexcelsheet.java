...
public class Writecodetoopenexcelsheet {
    ...
    // How to read Excel files using Apache poi	
		File fil =new File("C:\\Swati\\Excel\\newlinkedin1.xlsx"); 
	...
	//		System.out.println(Data9);
			...
			for(int i=0;i<rowcount;i++) {
				...
				File fil1 =new File("C:\\Swati\\Excel\\newlinkedin1.xlsx"); 
				//FileOutputStream fout= new FileOutputStream(fil1);
				XSSFWorkbook workbookOut = new XSSFWorkbook(fil1);
				XSSFSheet sheetOut = workbook.getSheet("Sheet1");
				for(int k=0;k<rowcount;k++) {
			 
				sheetOut.getRow(k).createCell(3).setCellValue("Data Imported");
				workbookOut.write(fil1);
				workbookOut.close();
				}
